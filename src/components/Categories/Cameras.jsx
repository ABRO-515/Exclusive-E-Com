...
import TotalCameras from "../../api/TotalCameras";
...
const searchCamera = (elem) => {
  ...
  const filteredCameras = TotalCameras.filter((elem)=> searchCamera(elem))
    ...
    return (
      <>
        ...
        <div className="flex gap-3 items-center" >
          ...
          <h3 className="text-red-500 font-[400] " >Our Cameras</h3>
        </div>
        <h1 className="font-bold pt-5 text-2xl lg:text-3xl" >Explore Our Cameras</h1>
      </div>

  {filteredCameras.length === 0 ?  (
    <div className="flex items-center min-h-screen justify-center w-full mx-auto" >
  <h1 className="text-2xl" >Camera not found</h1>
    </div>
  ) :  
  ...
  {filteredCameras.map((product) => (
    <NavLink to={`/products/${product.title}`}><div
       ...
       >
         ...
         <div className="p-4">
           {/* Title */}
           <h3 className="font-semibold text-lg mb-2">{product.title.length > 16 ? product.title.slice(0,16) + "..." : product.title}</h3>
