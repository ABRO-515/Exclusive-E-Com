...
const searchProducts = (elem) => {
  ...
  const filteredProducts = TotalProducts.filter((elem)=> searchProducts(elem))
  ...
  const handleProductClick = () => {
    ...
    return (
      <>
        ...
        <h1 className="font-bold pt-5 text-2xl lg:text-3xl" >Explore Our All Products</h1>
      </div>
      ...
      <div className="  grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 mx-4 md:mx-6 xl:mx-12 mt-8 mb-16">
       ...
       <NavLink to={`/products/${product.title}`}><div
          ...
          >
            ...
            <div className="p-4">
              {/* Title */}
              <h3 className="font-semibold text-md mb-2">{product.title.length > 16 ? product.title.slice(0,16) + "..." : product.title}</h3>
